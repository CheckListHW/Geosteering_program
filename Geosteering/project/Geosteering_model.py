...
class Geostering_Model():    
    def __init__(self, xml_path, GR_path):
        ...
        self.gamma_real['value'] = get_alpha(self.gamma_real['value'])
    ...
    def sint_curve_generator(self, start_offset_point, gamma_real_cut, delta, d):
        ...
        curvature = (real_tr - np.linspace(real_tr[0], real_tr[-1], len(real_tr)))/self.scale  # рассчитываем кривизну скважины на заданном участке        
        ...
        sintetic_curves = [np.linspace(start_offset_point,
                                       end_point,
                                       len(gamma_real_cut.MD)) + curvature for end_point in end_points]   # рассчитываем проекции получившейся траектории на опорный каротаж
    ...
    def save_results_to_xml(self, l=150):
        ...
        for x_top, y_top in zip(self.top_x[::l], self.top_y[::l]):
            new_top_markers.append(Point(x=x_top, y=y_top))

        for x_bot, y_bot in zip(self.bot_x[::l], self.bot_y[::l]):
            new_bot_markers.append(Point(x=x_bot, y=y_bot))

        top_surface_points = []
        for point in self.Cor.Section.Surfaces[0].Points:
            if (point.X + 5) < self.top_x[0]:
                top_surface_points.append(point)
            else:
                break
